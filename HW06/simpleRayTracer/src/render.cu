...
__global__ void renderKernel(const int NI,
  ...
  material_t *materials = scene.c_materials;
  shape_t    *shapes    = scene.c_shapes;
  light_t    *lights    = scene.c_lights;
  ...
  // int J = ythreadIndex + ythreadCount*yblockIndex;
	...
	colour_t newc =
	  gridTrace(grid, Nshapes, shapes, Nlights, lights, Nmaterials, materials, r, level, coef, bg);
