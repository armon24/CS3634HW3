...
scene_t *sceneSetup(){
  ...
  materials[0].eta = 1;
  materials[0].refraction = 1;

  materials[0].info.refractor = 0; //     ***changes refractor from 0 to 1***
  materials[0].info.reflector = 1; //     ***change reflector from 1 to 0****
  materials[0].info.emitter = 1;
  ...
  for(i=1;i<Nmaterials;++i){
    ...
    materials[i].diffuse.red   = .5; // drandRange48(0.125,0.8);
    materials[i].diffuse.green = .33;//drandRange48(0.125,0.8);
    materials[i].diffuse.blue  = .7234;//drandRange48(0.5,0.8);//0.125
  ...
  int Ncones = 10;    // 20 random cones
  int Ncylinders = 3; //20 random cylinders
  int Nrectangles = 1;// 1  ground plane rectangle
  int NDisks = 0;

  // need int NDisk and add it to Nshapes then copy and paste the cylinder for loop, deelete everything but change the end cap
 
  int Nshapes = Nspheres + Nbunny*Ntriangles + Ncones + 3*Ncylinders + Nrectangles + NDisks; // each cylinder has two end disks
  ...
  // {
     // end cap
      ...
      // shapes[cnt].disk.normal.x = 2*drand48()-1;
    //   shapes[cnt].disk.normal.y = ( 2*sqrt(1 - pow(shapes[cnt].disk.normal.x, 2)) ) * ( drand48() ) - sqrt(1 - shapes[cnt].disk.normal.x);

    // shapes[cnt].disk.normal.z = (  2*sqrt(1 - pow(shapes[cnt].disk.normal.x, 2) - pow(shapes[cnt].disk.normal.y, 2) ) * drand48() )
      //   - sqrt(1 - pow(shapes[cnt].disk.normal.x, 2) - pow(shapes[cnt].disk.normal.y, 2));
                                             

                                                                    // vectorScale(+1.,oldCylinder.axis);
    //** shapes[cnt].disk.center = 1000 * drand48();   //SOMETHING WRONG HERE

    //  shapes[cnt].disk.center.x = 2*drand48()-1;
    //  shapes[cnt].disk.center.y = ( 2*sqrt(1 - pow(shapes[cnt].disk.center.x, 2)) ) * ( drand48() ) - sqrt(1 - shapes[cnt].disk.center.x);

    //   shapes[cnt].disk.center.z = (  2*sqrt(1 - pow(shapes[cnt].disk.center.x, 2) - pow(shapes[cnt].disk.center.y, 2) ) * drand48() )
    //  - sqrt(1 - pow(shapes[cnt].disk.center.x, 2) - pow(shapes[cnt].disk.center.y, 2));
  ...
  for(i=0;i<Ncylinders;++i){
    ...
    shapes[cnt].cylinder.center.x = 1000 * drand48();
     shapes[cnt].cylinder.center.y = 1000 * drand48();
    shapes[cnt].cylinder.center.z = 1000 * drand48();
    ...
    */
     ...
     shapes[cnt].material = 50 + (Nmaterials-2)*drand48();
    ...
    //end cap
     shapes[cnt].disk.radius = oldCylinder.radius;  //100 + 10*drand48()
     shapes[cnt].disk.normal = vectorScale(+1.,oldCylinder.axis); // why ?
     shapes[cnt].disk.center = oldCylinder.center; //1000 * drand48();
        shapes[cnt].material = 10;
     shapes[cnt].type = DISK;
     shapes[cnt].id = cnt;
     ++cnt;
    ...
    // end cap (disk)
     ...
     shapes[cnt].disk.normal = vectorScale(-1.,oldCylinder.axis); //use axisDisk here

     shapes[cnt].disk.center =
      vectorAdd(oldCylinder.center, vectorScale(oldCylinder.height, oldCylinder.axis));
      // vectorAdd(centerDisk, vectorScale(75, centerDisk)); //create own value for height and use centerDisk for oldCYlinder
